...
export function OrdersPage({ setView: _setView, setSelected: _setSelected }: { setView: (view: View) => void; setSelected: (order: never) => void }) {
  ...
  const update = async (order: VendorOrder, next: 'pack' | 'ship') => { setAction(order._id); setError(''); try { if (next === 'pack') await orderService.pack(order._id); else await orderService.ship(order._id); load() } catch (cause) { setError(cause instanceof Error ? cause.message : 'Unable to update order') } finally { setAction(undefined) } }
