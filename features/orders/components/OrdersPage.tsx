...
export function OrdersPage({ setView: _setView, setSelected: _setSelected }: { setView: (view: View) => void; setSelected: (order: never) => void }) {
  ...
  const load = useCallback((signal?: AbortSignal) => {
    ...
    setError('')
    setRawError(null)
    orderService.list({ page, limit, status: filter, search }, { signal })
      .then((result) => {
  ...
  return (
    <>
      <PageHeader eyebrow="Fulfilment" title="Orders" description="Manage the orders assigned to your vendor account." />

      {loading && orders.length === 0 ? (
        <MetricCardsSkeleton count={4} />
      ) : (
        <div className="metrics-grid">
          <MetricCard label="Awaiting payment" value={String(counts.awaiting)} change="Current page" icon={ClipboardList} />
          <MetricCard label="Processing" value={String(counts.processing)} change="Current page" icon={Package} accent="blue" />
          <MetricCard label="In transit" value={String(counts.transit)} change="Current page" icon={Truck} accent="green" />
          <MetricCard label="Delivered" value={String(counts.delivered)} change="Current page" icon={Check} accent="purple" />
        </div>
      )}
      ...
      <section className="panel table-panel">
        <div className="tabs inventory-tabs">
          {statuses.map((item) => (
            <button
              key={item.label}
              className={filter === item.value ? 'active' : !filter && !item.value ? 'active' : ''}
        ...
        {loading && visible.length === 0 ? (
          <TableSkeleton columns={5} rows={6} hasCheckbox={false} />
        ) : error && visible.length === 0 ? (
          <ErrorState error={rawError || error} onRetry={() => load()} />
        ) : visible.length === 0 ? (
          <EmptyState
            title="No seller orders found"
            description="Orders assigned to your approved vendor account will appear here."
          />
        ) : (
            ...
            <table>
              ...
              <tbody>
                {visible.map((order) => (
                  <tr key={order._id}>
                    <td>
                      <button className="link-button" onClick={() => open(order._id)}>
                        #{order._id.slice(-8).toUpperCase()}
                      </button>
                      <small>{orderDate(order)}</small>
                    </td>
                    <td>
                      <b>{order.items[0]?.productName || 'Product unavailable'}</b>
                      <small>{order.items.length} item{order.items.length === 1 ? '' : 's'} · {order.items[0]?.sku || 'No SKU'}</small>
                    </td>
                    <td>
                      <b>{money(order.total)}</b>
                      <small>{order.items.reduce((sum, item) => sum + item.quantity, 0)} units</small>
                    </td>
                    <td>
                      <StatusBadge tone={order.parent?.paymentStatus === 'PAID' ? 'success' : 'warning'}>
                        {order.parent?.paymentStatus || 'PENDING'}
                      </StatusBadge>
                    </td>
                    <td>
                    ...
                    <td>
                      <button className="tiny-button" onClick={() => open(order._id)}>View</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="table-footer">
          <span>
            Showing <b>{visible.length ? `${(page - 1) * limit + 1}–${Math.min(page * limit, total)}` : '0'}</b> of {total} orders
            {loading && visible.length > 0 && <span style={{ marginLeft: '8px', color: 'var(--muted)' }}>(updating…)</span>}
          </span>
          <div>
